...
**Every command the chrome calls needs a line in `capabilities/default.json`,
...
Two things to know. The inspector opens in its own window and **takes focus**,
and WebKit offers no way to ask otherwise, so this is the one part of the app
that cannot stay behind what you are doing. And `devtools` is in the `tauri`
features in `Cargo.toml`: it is on automatically in a debug build and off in
release without it, so leaving it out would give a bundled app two dead
buttons.
